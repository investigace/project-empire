...
* ${media_mention.summary + ' ' if media_mention.summary else ''}[${media_mention.url} ${media_mention.url}]${' Naposledy kontrolováno / Last checked: ' + media_mention.last_checked_date.strftime('%Y-%m-%d') if media_mention.last_checked_date else ''}
% endfor
% endif
% if len(media_mentions) == 0:
